...
#ifndef _PLACE_B_PR2_
#define _PLACE_B_PR2_

class placeBehaviour : public pr2Behaviour{
public:
    placeBehaviour(std::string instance){
    ...
    std::string oprs_message(){
        std::stringstream ss;
        ss<<"( AttentionalInterface.execute place (. "<<SEED2laas_param(target)<<" "<<SEED2laas_param(position)<< " .) "<<mpSender<< " )";
        return ss.str();
    }
    void update_wmv() {
